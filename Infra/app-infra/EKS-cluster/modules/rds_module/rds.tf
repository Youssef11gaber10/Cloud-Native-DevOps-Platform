...
resource "aws_db_subnet_group" "subnet_group_db_mysql" {
  ...
  subnet_ids = var.NM_private_subnets_ids
...
resource "aws_db_instance" "db_instnace" {
  ...
  db_name = var.db_name # mysql-db
  # username 
  username = var.db_user_username #admin # make it as variable also , the guy want to use module just provide username, password and i will create db for him
  # password 
